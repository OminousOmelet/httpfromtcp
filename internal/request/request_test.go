...
func TestRequestLineParse(t *testing.T) {
	...
	_, err = RequestFromReader(strings.NewReader(
		"/coffee HTTP/1.1 POST\r\nHost: localhost:42069\r\nUser-Agent: curl/7.81.0\r\nAccept: */*\r\n\r\n"))
	require.Error(t, err)

	// Test: Invalid method (non-alphabetic) Request line
	_, err = RequestFromReader(strings.NewReader(
		"/coffee HTTP/1.1 1.1\r\nHost: localhost:42069\r\nUser-Agent: curl/7.81.0\r\nAccept: */*\r\n\r\n"))
	require.Error(t, err)
...
func TestHeadersParse(t *testing.T) {
	...
	// Test: Empty Headers, double-check request line
	reader = &chunkReader{
		data:            "GET /fkinTea HTTP/1.1\r\n\r\n\r\n",
		numBytesPerRead: 3,
	}
	r, err = RequestFromReader(reader)
	require.NoError(t, err)
	require.NotNil(t, r)
	assert.Equal(t, "GET", r.RequestLine.Method)
	assert.Equal(t, "/fkinTea", r.RequestLine.RequestTarget)
	assert.Equal(t, "1.1", r.RequestLine.HttpVersion)
